require 'json'
  ...
  #puts alljson
  #fin = Time.now + 5 * 60; while Time.now < fin; puts fin  - Time.now; sleep 1; end
  ...
  start = false
    ...
    puts " The timer will start Immediately "
  ...
  while !start
    ...
    if qsn.between?(0,5)
      ...
      if "D" == myans.upcase
        myscore +=1
        puts "correct"
        puts "Now select the next question with the number keys"
      else
        puts "wrong"
        puts "Now select the next question with the number keys"
      end
